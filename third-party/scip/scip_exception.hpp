...
// unfortunately SCIP has no method to get the string of an error code, you can
// just print it to a file so we add such a method here, this has to be updated
// when SCIP Messages changes currently supporting SCIP-1.00
 ...
 * @param[out] buffer_str buffer to character array to store translated message,
 * this must be at least of size SCIP_MSG_MAX
 * @return buffer_str or NULL, if retcode could not be translated
 */
inline char*
SCIPgetErrorString(SCIP_RETCODE retcode, char* buffer_str, int buffersize)
{
    ...
    switch (retcode) {
        ...
        case SCIP_NOMEMORY:
            (void)SCIPsnprintf(
              buffer_str, buffersize, "insufficient memory error");
            return buffer_str;
        case SCIP_READERROR:
        ...
        case SCIP_INVALIDRESULT:
            (void)SCIPsnprintf(
              buffer_str, buffersize, "method returned an invalid result code");
            return buffer_str;
        case SCIP_PLUGINNOTFOUND:
            (void)SCIPsnprintf(
              buffer_str, buffersize, "a required plugin was not found");
            return buffer_str;
        ...
        case SCIP_MAXDEPTHLEVEL:
            (void)SCIPsnprintf(
              buffer_str, buffersize, "maximal branching depth level exceeded");
            return buffer_str;
        case SCIP_NOTIMPLEMENTED:
            (void)SCIPsnprintf(
              buffer_str, buffersize, "function not implemented");
            return buffer_str;
    }
    return NULL;
}
 ...
 *
 * this class enables you to handle the return code of SCIP functions in a C++
 * way
 */
  ...
  public:
      ...
      : _retcode(retcode)
    {
        if (SCIPgetErrorString(retcode, _msg, SCIP_MSG_MAX) == NULL)
            (void)SCIPsnprintf(
              _msg, SCIP_MSG_MAX, "unknown SCIP retcode %d", retcode);
    }
     ...
     *
     * this overrides the corresponding method of std::exception in order to
     * allow you to catch all of your exceptions as std::exception
     */
